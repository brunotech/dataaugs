...
class NFNet(nn.Module):
    def __init__(self, channels: int, classes: int, variant: str = 'F0', stochdepth_rate: float = None,
                 alpha: float = 0.2, se_ratio: float = 0.5, activation: str = 'gelu', stem: str = 'ImageNet',
                 use_dropout=False):
    ...
    def forward(self, x):
        ...
        if self.training and self.drop_rate > 0. and self.use_dropout:
...
class NFBlock(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, expansion: float = 0.5,
                 ...
                 group_size: int = 1, stochdepth_rate: float = None, activation: str = 'gelu', pad: int = 1):
        ...
        self.conv1 = WSConv2D(in_channels=self.width, out_channels=self.width,
                              kernel_size=3, stride=stride, padding=1, groups=self.groups)
        self.conv1b = WSConv2D(in_channels=self.width, out_channels=self.width,
                               kernel_size=3, stride=1, padding=1, groups=self.groups)
...
class WSConv2D(nn.Conv2d):
    def __init__(self, in_channels: int, out_channels: int, kernel_size, stride=1, padding=0,
        ...
        self.register_buffer('fan_in', torch.tensor(
            np.prod(self.weight.shape[1:]), requires_grad=False).type_as(self.weight), persistent=False)
